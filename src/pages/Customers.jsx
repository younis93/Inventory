...
import { isWithinInterval, parseISO, subDays, startOfDay, endOfDay, format } from 'date-fns';
...
const Customers = () => {
    const { customers, orders, addCustomer, updateCustomer, formatCurrency, brand, addToast } = useInventory();
    ...
    }, [customers, searchTerm, filterGovernorates, filterSocials, sortBy, columnSort, orders, dateRange]);
            ...
            <div className="flex flex-col gap-4 mb-8 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <div className="flex gap-3 w-full items-center justify-between flex-wrap">
                    <div className="flex gap-3 items-center flex-wrap">
                        ...
                        <button
                            onClick={() => {
                                if (filteredAndSortedCustomers.length === 0) return addToast("No customers matching filters to export", "info");
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                    ...
                                    <th className="px-6 py-4 border-l dark:border-slate-700 select-none">Created On</th>
                                        ...
                                        <tr key={customer._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 border-l dark:border-slate-700">
                                                <span className="text-xs font-bold text-slate-500 whitespace-nowrap">
                                                    {getValidDate(customer) ? format(getValidDate(customer), 'MMM dd, yyyy') : 'N/A'}
                                                </span>
                                            </td>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
                        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-white dark:bg-slate-800 sticky top-0 z-10">
                            <div>
                                ...
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Customer Database</p>
                            </div>
                        ...
                        <div className="p-6 pt-0 mt-auto border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800">
                            <div className="flex gap-4 pt-4">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 border-2 border-slate-100 dark:border-slate-800 rounded-2xl font-black text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95">Cancel</button>
                                <button
